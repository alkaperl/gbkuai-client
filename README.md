# GBkuai Client
[![Build Status](https://travis-ci.org/alkaperl/gbkuai-client.svg?branch=master)](https://travis-ci.org/alkaperl/gbkuai-client)

The GBkuai Client is a cross-platform VPN or proxy client for Windows, macOS, iOS, Android, and ChromeOS.  The GBkuai Client is designed for use with the [GBkuai Server](https://github.com/alkaperl/gbkuai-server) software, but it is fully compatible with any [Shadowsocks](https://shadowsocks.org/) server that has UDP support enabled.
...
GBkuai clients shares the same web app across all platforms.  This code is located in the www directory.  If you are making changes to the shared web app and do not need to test platform-specific functionality, you can test in a desktop browser by running:
